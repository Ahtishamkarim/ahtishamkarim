<div align="center">

# Hi there, I'm Ahtisham Karim 👋

### Software Engineer | Full-Stack Developer (MERN / Next.js / AI)

Building scalable web applications and AI-integrated systems with React.js, Next.js, Node.js, NestJS, Express.js, PHP, MongoDB, MySQL & PostgreSQL.

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://ahtisham-portfolio-2q7a.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ahtisham-karim)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ahtisham.karim8391@gmail.com)
[![Location](https://img.shields.io/badge/Lahore-Pakistan-2ea44f?style=for-the-badge&logo=googlemaps&logoColor=white)](#)

</div>

---

## 🚀 About Me

I'm a Software Engineer with hands-on experience building scalable web applications end-to-end — from responsive front-end interfaces to back-end APIs, databases, AI integrations, and deployment. I enjoy solving real problems, working with international clients, and shipping products that people actually use. Strong problem solver, fast learner, and team player focused on meeting goals and deadlines.

---

## 🌐 Portfolio

A closer look at my work, selected projects, and what I can help you build.

**[View Portfolio](https://ahtisham-portfolio-2q7a.vercel.app/)**

---

## 🛠️ Tech Stack

**Frontend**

<img src="https://skillicons.dev/icons?i=html,css,tailwind,js,ts,react,nextjs" alt="HTML5, CSS3, Tailwind CSS, JavaScript, TypeScript, React, Next.js" />

**Backend**

<img src="https://skillicons.dev/icons?i=nodejs,express,nestjs,php" alt="Node.js, Express.js, NestJS, PHP" />

**Artificial Intelligence**

<img src="https://skillicons.dev/icons?i=openai" alt="OpenAI" /> <a href="https://gemini.google.com"><img src="https://cdn.simpleicons.org/googlegemini/8E75B2" height="48" alt="Gemini" /></a> <a href="https://claude.ai"><img src="https://cdn.simpleicons.org/anthropic/D97757" height="48" alt="Claude" /></a>

**Databases**

<img src="https://skillicons.dev/icons?i=mongodb,mysql,postgres" alt="MongoDB, MySQL, PostgreSQL" />

**CMS & E-commerce**

<img src="https://skillicons.dev/icons?i=wordpress" alt="WordPress" /> <a href="https://shopify.com"><img src="https://cdn.simpleicons.org/shopify/96BF48" height="48" alt="Shopify" /></a>

**Tools & Hosting**

<img src="https://skillicons.dev/icons?i=git,github,docker,figma,vercel" alt="Git, GitHub, Docker, Figma, Vercel" /> <a href="https://hostinger.com"><img src="https://cdn.simpleicons.org/hostinger/673DE6" height="48" alt="Hostinger" /></a>

---

## 💼 Professional Experience

**Full-Stack Developer**  
GoodToGo

**Frontend Web Developer**  
ByteForce IT Solutions

**Full-Stack Developer**  
Freelancer.com

**Frontend Web Developer**  
Devsinc

---

## 🧩 Featured Projects

### 🔹 [ZhengHe](https://zh.com.sg)
ZhengHe connects job seekers and employers through a clear and well-organized hiring system, focusing on verified profiles, accurate job matching, and streamlined recruitment workflows.

### 🔹 [CiteReady](https://tryciteready.com)
An AI-powered website auditing platform that analyzes how well a site performs in AI-driven search. It crawls and scores sites across four pillars — access, extractability, trust, and citation — then probes live AI engines (ChatGPT, Perplexity, Gemini, Claude, Copilot, and Grok) to check whether pages are actually being cited in AI-generated answers, delivering a ranked report with a prioritized action plan.

### 🔹 [StrangerUs](https://strangerus.com)
A next-generation social discovery platform that enables people to create genuine, meaningful connections in the real world, going beyond typical online interactions.

### 🔹 [My Easy Junk Removal](https://myeasyjunkremoval.com)
A nationwide junk removal marketplace for the United States. Customers post a job with photos/videos of their junk, and registered local service providers respond with competitive quotes — customers compare offers and book same-day or next-day pickup.

### 🔹 [Baqi Digital](https://baqidigital.com)
A systems-driven digital growth agency focused on helping businesses scale with clarity, discipline, and measurable execution.

---

## 🎓 Education

**BS Software Engineering** — Lahore Garrison University &nbsp;|&nbsp; *10/2020 – 07/2024* &nbsp;|&nbsp; Lahore, Pakistan

---

## 🌐 Languages

Urdu (Native) &nbsp;•&nbsp; English (B2 — Independent User)

---

## 📫 Let's Connect

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://ahtisham-portfolio-2q7a.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ahtisham-karim)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ahtisham.karim8391@gmail.com)

<div align="center">
<sub>Thanks for stopping by! ⭐ this profile if you found something useful.</sub>
</div>
